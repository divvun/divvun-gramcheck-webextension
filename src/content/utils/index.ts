...
export function clipToErrorContext(
  parargaph: string,
  errorText: string,
  errorOffset: number,
): string {
...
function loadIgnoredErrorTags(): string[] {
  const savedIgnoredErrorTags = loadSettings(IGNORED_ERROR_TAGS_KEY);
...
function loadIgnoredIndividualErrors(): string[] {
  const savedIgnoredErrors = loadSettings(IGNORED_ERRORS_KEY);
